    ...
    return (lines || [])
        .reduce((a: any[], line) => {
            ...
            if (prevLine) {
                ...
                if (config.insertLineAfterBlock && prevLine.text.trim() === '}' && !line.isEmptyOrWhitespace &&
                    !beginsWithClosingBracket(line.text) && !beginsWithClosingTag(line.text)) { a.push(null); }
            }
...
function beginsWithClosingTag(text: string) {
